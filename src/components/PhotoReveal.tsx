...
const PhotoReveal = forwardRef<HTMLDivElement, Props>(({ revealed }, ref) => {
  ...
  return (
    ...
    >
      <div
        className={`transition-all duration-[2000ms] ease-out ${revealed ? "opacity-100 scale-100" : "opacity-0 scale-75"}`}
      >
        <div className="relative">
          ...
          >
            <div
              className="w-full h-full rounded-[calc(1.8rem-2px)] overflow-hidden relative"
        ...
        {/* Text below photo */}
        <div
          className={`text-center mt-10 transition-all duration-1000 delay-[1500ms] ${revealed ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          <p className="font-panorama text-3xl md:text-4xl" style={{ color: "hsl(var(--secondary) / 0.7)" }}>
            عيد ميلاد سعيد يا أحلى شروق
          </p>
