...
const HeroSection = ({ onReady }: Props) => {
  ...
  useEffect(() => {
    const timers = [
      setTimeout(() => setStage(1), 300),   // "19" appears
      setTimeout(() => setStage(2), 1200),   // subtitle
      setTimeout(() => setStage(3), 1800),   // name starts typing
      setTimeout(() => setStage(4), 3200),   // Arabic name
      setTimeout(() => setStage(5), 3800),   // bottom shimmer
      setTimeout(() => { setStage(6); onReady?.(); }, 4400), // scroll hint
    ];
      ...
      <div className="text-center">
        ...
        <div className={`transition-all duration-1000 ${stage >= 2 ? "opacity-100 scale-x-100" : "opacity-0 scale-x-0"}`}>
          <div className="flex items-center justify-center gap-3 mb-6">
            ...
            <span className="text-secondary/60 text-xs tracking-[0.6em] uppercase font-light">Happy Birthday</span>
            <div className="w-12 h-[1px] animate-shimmer rounded-full" />
          </div>
        </div>
        ...
        <div className={`transition-all duration-700 ${stage >= 3 ? "opacity-100" : "opacity-0"}`}>
          ...
          {stage >= 3 && nameChars < name.length && (
            <span className="inline-block w-[3px] h-14 bg-secondary/60 ml-1 animate-pulse align-bottom" />
          )}
        </div>
        ...
        <div className={`transition-all duration-1000 ${stage >= 4 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
          <div className="relative inline-block mt-3">
            ...
            <p className="relative font-arabic text-4xl md:text-5xl text-secondary/80 font-bold">شروق</p>
          </div>
        </div>
        ...
        <div className={`transition-all duration-1000 ${stage >= 5 ? "opacity-100" : "opacity-0"}`}>
          ...
          <p className="font-arabic text-sm text-muted-foreground/40">
        ...
        <div className={`mt-16 transition-all duration-1000 ${stage >= 6 ? "opacity-100" : "opacity-0"}`}>
          <div className="flex flex-col items-center gap-2">
            <div className="w-5 h-9 rounded-full border border-muted-foreground/15 flex items-start justify-center p-1.5">
              <div className="w-1 h-2 rounded-full bg-secondary/40 animate-scroll-dot" />
            </div>
          </div>
        </div>
      </div>
